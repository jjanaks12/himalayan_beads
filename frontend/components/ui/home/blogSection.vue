    ...
    <div class="container mx-auto px-4">
      ...
      <div class="blog__posts__grid grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        ...
        <NuxtLink
          ...
          :key="post.id"
          :to="`/blog/${post.slug}`"
          class="blog__post__card rounded-lg transition-all duration-500 ease-out group overflow-hidden block"
          :class="{
            ...
            <div class="blog__content p-6 relative flex-grow flex flex-col justify-between">
              <div>
                <div class="blog__meta flex items-center gap-2 mb-3">
                  <span class="text-sm bg-[#F6EEEA] py-2 px-3 rounded-sm text-[#3E4E5B] font-medium">{{ post.category }}</span>
                  <span class="text-gray-300">•</span>
                  <span class="text-sm text-gray-500">{{ post.timeAgo }}</span>
                </div>
                <h3 class="blog__title text-lg lg:text-xl font-semibold text-gray-800 mb-3 leading-tight group-hover:text-[#804224] transition-colors duration-300">
                  {{ post.title }}
                </h3>
              ...
              <div class="read__more__container relative h-10 mt-auto">
                <div class="read__more__button absolute inset-0 flex items-center">
                  <span class="read__more__btn opacity-0 bg-white font-bold border border-[#804224] group-hover:bg-[#804224] group-hover:text-white text-[#804224] px-4 py-2 rounded text-xs font-medium transition-all duration-300 flex items-center gap-1">
                    READ MORE
                  </span>
                </div>
              </div>
...
*/
.blog__posts__section { position: relative; }
.blog__post__card { transition: all 0.3s cubic-bezier(0.4, 0, 0.2, 1); }
.blog__image { position: relative; background: linear-gradient(135deg, #fafafa 0%, #f5f5f5 100%); }
.blog__content { position: relative; }
.read__more__container { position: relative; min-height: 2.5rem; }
.read__more__button { transition: all 0.4s cubic-bezier(0.4, 0, 0.2, 1); }
.group:hover .blog__description { color: #4b5563; }
.group:hover .read__more__btn { animation: fadeIn 0.4s ease-out forwards; animation-delay: 0.1s; transition: 0.3s ease-in-out; }
@keyframes fadeIn { from { opacity: 0; } to { opacity: 1; } }
@media (max-width: 768px) { .blog__posts__grid { grid-template-columns: 1fr; gap: 1.5rem; } .blog__content { padding: 1.25rem; } .blog__image img { height: 12rem; } }
@media (min-width: 769px) and (max-width: 1023px) { .blog__posts__grid { grid-template-columns: repeat(2, 1fr); gap: 1.5rem; } }
.transition-all { transition-property: all; transition-timing-function: cubic-bezier(0.4, 0, 0.2, 1); }
.blog__image img { will-change: transform; }
</style>