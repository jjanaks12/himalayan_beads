...
<template>
  <section class="promotion__banner py-4 relative overflow-hidden cursor-pointer" @click="handleSignUp"
    ...
    <div class="container mx-auto px-4">
      <div class="flex items-center justify-center gap-3 text-center transition-all duration-700 ease-out" :class="{
        'opacity-100 translate-y-0': isVisible,
        'opacity-0 translate-y-4': !isVisible,
      }">
        <!-- Lightning Icon -->
        <ZapIcon class="h-5 w-5 text-[#FBB530] transition-all duration-300" :class="{
        ...
        <!-- Down Arrow -->
        <ArrowDownIcon class="h-4 w-4 text-[#B7B5B4] transition-all duration-500" :class="{
