  ...
  <section ref="sectionRef" class="cta__video__section relative bg-[#100804] lg:py-0">
    ...
    <div class="container mx-auto px-4 py-16 lg:py-0">
      <div class="grid grid-cols-1 lg:grid-cols-5 gap-8 lg:min-h-[600px]">
        ...
        <div class="content__column lg:col-span-2 flex flex-col justify-center transition-all duration-800 ease-out"
          ...
          <!-- <div class="cta__button transition-opacity duration-800"
            :class="{ 'opacity-100': isVisible, 'opacity-0': !isVisible }" :style="{ transitionDelay: '1000ms' }">
            <button
              class="learn__more__btn bg-[#A0522D] text-white px-8 py-4 rounded-sm font-semibold uppercase tracking-wide transition-transform duration-300 hover:scale-105">
    ...
    <div
      ...
      <div ref="videoContainerRef" class="video__container relative w-full h-full cursor-pointer group"
        @click="toggleVideo">
        <video ref="videoRef" class="w-full h-full object-cover" muted loop playsinline preload="metadata"
          poster="/images/Video.png">
          <source src="/videos/demo.mp4" type="video/mp4" />
        </video>

        <!-- Overlay -->
        <div class="video__overlay absolute inset-0 bg-black/20 transition-opacity duration-300"
          :class="isPlaying && !isHoveringVideo ? 'opacity-0' : 'opacity-100'"></div>
        ...
        <div
          ...
          <div
            class="control__button bg-white/80 rounded-full p-4 lg:p-6 shadow-xl backdrop-blur-sm transition-transform duration-300 group-hover:scale-110">
...
<style scoped>
  .cta__video__section {
    overflow-x: clip;
    /* Prevents horizontal scrollbar from animation */
  }

  /* This is key for the mobile layout */
  .video__column {
    min-height: 400px;
    /* Ensures video has a good height on mobile */
  }
